...
/* [실행 시] 플레이어 위치(playerX, playerY)를 시작점으로 BFS(너비우선탐색)를 수행한다.
   ① 맵 전체를 ENEMY_UNREACHABLE로 초기화한다.
   ② 플레이어 칸을 거리 0으로 설정하고 큐에 삽입한다.
   ③ 큐에서 꺼낸 칸의 상하좌우를 탐색하며 벽이 아니고 아직 더 짧은 거리가
      없는 칸에 거리를 기록하고 큐에 추가한다.
   ④ 큐가 비면 distances 배열에 각 칸에서 플레이어까지의 최단 거리가 채워진다.
...
/* [실행 시] 적의 현재 위치에서 8방향을 모두 검사한다. (BFS)
   ① 맵 경계 밖·벽·다른 적이 있는 칸·도달 불가 칸은 건너뛴다.
   ② 유효한 칸 중 distances 값이 가장 작은 칸(플레이어에 가장 가까운 칸)을 선택한다.
   ③ 이동 가능한 칸이 있으면 적의 position을 해당 칸으로 갱신하고 true를 반환한다.
   ④ 이동할 곳이 없으면 false를 반환한다. */
...
/* [실행 시] (dx, dy) 방향으로 이동할 다음 칸을 계산한다.
   ① 맵 경계를 벗어나지 않도록 클램프(값을 특정 범위안에 가두기)한다.
   ② 다음 칸이 벽이 아니면 플레이어 position을 해당 칸으로 갱신한다.
   ③ 벽이면 이동하지 않는다. */
...
/* [실행 시] elapsed를 90으로 나눈 단계 수로 속도/HP 배율을 산출한다. (1분 30초마다 단계 증가)
   ① 단계당 HP는 +10%, 속도는 +3% 증가(최대 200% 유지)한다.
   ② type에 따라 박쥐(b)·좀비(G)·뱀파이어(V) 중 하나의 Enemy 구조체를 반환한다.
...
/* [실행 시] 방패 타이머와 히트 카운트를 0으로 초기화한다.
   ① shieldBreakTimer를 1.2초로 설정해 폭발 연출을 시작한다.
   ② 활성화된 모든 적을 EnemyDefeat로 제거한다.
...
/* [실행 시] dt만큼 이동 쿨타임과 무적 타이머를 감소시킨다.
   ① 회복의 붕대가 활성 중이면 활성 창 타이머를 감소하고 만료 시 붕대를 제거한다.
   ② 무적의 방패가 활성 중이면 방패 타이머를 감소하고 만료 시 ShieldBreak를 호출한다.
   ③ 입력(left/right/up/down)이 있고 이동 쿨타임이 0 이하면 TryMovePlayer를 호출한다.
   ④ 이동 후 이동속도 배율을 반영한 새 쿨타임을 설정한다.
...
/* [실행 시] 목표 칸이 벽이거나 이미 적이 있으면 false를 반환한다.
   ① 비어있는 enemies 슬롯을 탐색한다.
   ② 슬롯이 없으면 false를 반환한다.
   ③ 슬롯이 있으면 CreateEnemy로 적을 생성하고 true를 반환한다. */
...
/* [실행 시] 빈 enemies 슬롯이 없으면 즉시 반환한다.
   ① elapsed와 확률로 ENEMY_THREE_HP 또는 ENEMY_FORTY_HP 중 타입을 결정한다.
   ② 플레이어에서 최소 거리(10) 이상인 맵 외곽 위치를 최대 20회 시도해 찾는다.
   ③ 유효한 위치에 CreateEnemy로 적을 생성하고 슬롯에 저장한다.
...
/* [실행 시] count번 반복하며 매번 맵 외곽 4면 중 하나를 무작위로 선택한다.
   ① 선택된 면의 무작위 위치에 SpawnEnemyAt으로 ENEMY_ONE_HP(박쥐) 소환을 시도한다.
   ② 슬롯이 없거나 칸이 막혔으면 해당 반복만 건너뛴다.
...
/* [실행 시] 플레이어 킬 수를 1 증가시킨다.
   ① 회복의 붕대가 활성 중이면 bandageKillCount를 1 증가시킨다.
   ② bandageKillCount가 20에 도달하면 초기화하고 체력이 최대치 미만이면 1 회복한다.
...
/* [실행 시] 이미 비활성 적이면 즉시 반환한다.
   ① 적의 위치에 XP pickup을 생성한다.
   ② 1000분의 55 확률로 보물상자 pickup도 추가로 생성한다.
   ③ 적의 scoreValue를 플레이어 score에 더한다.
   ④ ApplyKillProgress로 킬 수 및 붕대 회복을 처리한다.
   ⑤ 적을 비활성화한다.
...
/* [실행 시] 모든 활성 적을 순회하며 center까지의 거리 제곱을 계산한다.
   ① radius 제곱 이내에 있는 적에게 damage만큼 체력을 감소시킨다.
   ② 체력이 0 이하가 되면 EnemyDefeat를 호출한다. */
...
/* [실행 시] MAX_PICKUPS 슬롯을 전부 순회하며 빈 슬롯을 찾는다.
   ① emptyTarget: 비어있는 슬롯 (최우선 사용).
   ② 슬롯이 꽉 찼으면 NULL 반환 (생성 안 함).
...
/* [실행 시] FindReusablePickupSlot으로 사용 가능한 슬롯을 얻는다.
   ① 슬롯이 NULL이면(병합·즉시 지급 처리됨) 반환한다.
   ② 슬롯이 있으면 active, type, position, value, moveCooldown을 설정한다. */
...
/* [실행 시] type에 따라 분기한다.
   ① PICKUP_XP: player.xp와 player.score에 value를 더하고 XP 획득음을 재생한다.
   ② PICKUP_TREASURE_CHEST: GameStartRewardChoice를 호출해 보상 선택 화면을 시작한다.
...
/* [실행 시] 모든 활성 pickup을 순회한다.
   ① 플레이어와의 거리 제곱이 PICKUP_COLLECT_DISTANCE² 이하이면 CollectPickup을 호출한다.
   ② XP pickup이 자석 범위 이내이고 moveCooldown이 0 이하이면
      플레이어 방향으로 한 칸 이동시키고 cooldown을 0.07초로 재설정한다.
   ③ 이동 목표 칸이 벽이면 이동하지 않는다.
